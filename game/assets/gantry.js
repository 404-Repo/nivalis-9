// NIVALIS 9 code asset | gantry | construction 2
...
export default function generate(THREE) {
  ...
  B(0,3.02,0,4,.3,14,dark);B(0,3.195,0,3.9,.05,13.9,steel);
  for(const x of [-1.87,1.87]) for(const z of [-6.3,0,6.3]) {B(x,1.5,z,.26,3,.3,steel);B(x,.08,z,.7,.16,.7,dark);}

  for(const x of [-1.95,1.95]) {C(x,4.27,0,.055,.055,14,orange,8,Math.PI/2);C(x,3.75,0,.035,.035,14,steel,8,Math.PI/2);for(let z=-6.8;z<7;z+=1.7)B(x,3.73,z,.09,1.1,.095,steel);}
  for(const x of [-1.84,1.84])for(const z of [-3.15,3.15]) {beam([x,.2,z-2.65],[x,2.81,z],.08,steel);beam([x,2.81,z],[x,.2,z+2.65],.08,steel);}
  for(const x of [-1.4,1.4])C(x,2.5,0,.13,.13,14,orange,12,Math.PI/2);

  for(let z=-6.8;z<7;z+=.22)B(0,3.231,z,3.76,.018,.048,dark);
  for(const x of [-1.75,1.75])B(x,3.255,0,.22,.04,13.95,snow);
  for(const z of [-5.5,5.5])B(0,2.85,z,1.8,.06,.15,warm);
